                    ...
                    budgets.map((budget) => (
                        ...
                        >
                            <p className="font-semibold">
                                Cliente: {budget.bike_id?.current_owner_id?.name}{" "}
                            ...
                            <div className="mt-3">
                                {budget.services
                                    ...
                                    .map((s) => {
                                        const nextCheckup = s.warranty.checkups?.find(
                                            (c) => !c.completed
                                        );
                                        let warning = "";
                                        ...
                                        if (nextCheckup) {
                                            const diffHours = Math.ceil(
                                                (new Date(nextCheckup.date) - now) /
                                                    (1000 * 60 * 60)
                                            );
                                            if (diffHours <= 1 && diffHours >= 0) {
                                                warning = `⚠️ Revisión en ${diffHours} horas`;
                                            }
                                        }
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p>
                                                    Garantía:{" "}
                                                    {new Date(s.warranty.startDate).toLocaleString()}{" "}
                                                    → {new Date(s.warranty.endDate).toLocaleString()}
                                                </p>

                                                {nextCheckup && (
                                                    <p>
                                                        Próximo checkup:{" "}
                                                        {new Date(nextCheckup.date).toLocaleString()}
                                                    </p>
                                                )}
